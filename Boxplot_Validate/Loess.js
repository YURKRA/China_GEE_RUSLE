...
/**** Start of imports. If edited, may not auto-convert in the playground. ****/
var ProvinceList = ['Gansu', 'Henan', 'Neimenggu','Ningxia',
'Qinghai','Shan_xi', 'Shanxi']

var blank_provinces = ['Aomen', 'Taiwan']
...
for(var year=2015; year<=2015 ;year++){
  ...
  for(var j=0;j<ProvinceList.length;j++){
    ...
    var table = ee.FeatureCollection("users/FJX/China_Data/Loess/"+loc + "_LoessPlateau");
    ...
    *****************************************************************/
    ...
    minList = minList.add(min)
    maxList = maxList.add(max);
    per_25_list = per_25_list.add(p25)
    per_75_list = per_75_list.add(p75)
    medianList = medianList.add(median)
  }
  
}
var min = minList.reduce(ee.Reducer.mean());
var per_25 = per_25_list.reduce(ee.Reducer.mean())
var median = medianList.reduce(ee.Reducer.mean())
var mean = meanList.reduce(ee.Reducer.mean());
var per_75 = per_75_list.reduce(ee.Reducer.mean())
var max = maxList.reduce(ee.Reducer.mean());
...
Export.table.toDrive({
    ...
    fileNamePrefix: year-1+'_Loess_New',
    description: year-1+'_Loess_New',
    folder:'Loess_Plot',
    fileFormat: 'CSV'
  });  
