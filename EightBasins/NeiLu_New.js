...
var ProvinceList = ['Gansu', 'Hebei', 'Neimenggu', 'Qinghai', 'Xinjiang',
'Xizang']
...
for(var year=2010;year<=2022;year++){
  ...
  for(var j=0; j<ProvinceList.length; j++){
    ...
    var regionList = ee.List([])

    var loc = ProvinceList[j]
    
    var table = ee.FeatureCollection("users/FJX/Province-Basin/NeiLu/"+ loc +"_NL");
    
    /*** split the roi ***/
    var grid = gridSplitMN(table.geometry(), 2, 1)
    ...
    *****************************************************************/
      ...
      regionList = regionList.add(mean.get('A'))
    }
    
    var mean = regionList.reduce(ee.Reducer.mean())
  ...
  Export.table.toDrive({
      ...
      fileNamePrefix: year+'_NL'+'_mean',
      description: year+'_NL'+'_mean',
      folder:'NL_New',
      fileFormat: 'CSV'
    });
  print("NeiLu Basin: ", year)
